...
mod tests {
    ...
    fn test_serde_serialize() {
        assert_eq!(
            serde_json::to_string(&Utc.at_ymd_and_hms(2014, 7, 24, 12, 34, 6).unwrap()).ok(),
    ...
    fn test_serde_deserialize() {
        ...
        assert_eq!(norm(&dt), norm(&Some(Utc.at_ymd_and_hms(2014, 7, 24, 12, 34, 6).unwrap())));
        ...
        assert_eq!(
            norm(&dt),
            norm(&Some(
                FixedOffset::east(0).unwrap().at_ymd_and_hms(2014, 7, 24, 12, 34, 6).unwrap()
            ))
        );
        let dt: Option<DateTime<FixedOffset>> =
        ...
        #[cfg(feature = "clock")]
        {
            ...
            assert_eq!(dt, Utc.at_ymd_and_hms(2014, 7, 24, 12, 34, 6).unwrap());
