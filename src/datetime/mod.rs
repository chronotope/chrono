...
impl<Tz: TimeZone> DateTime<Tz> {
    ...
    ///
    /// let dt = NaiveDate::from_ymd(1970, 1, 1)?.and_hms_milli(0, 0, 1, 444)?.and_local_timezone(Utc)?;
    ...
    /// use chrono::{Utc, TimeZone, NaiveDate};
    ///
    /// let dt = NaiveDate::from_ymd(1970, 1, 1)?.and_hms_micro(0, 0, 1, 444)?.and_local_timezone(Utc)?;
    ...
    /// use chrono::{Utc, TimeZone, NaiveDate};
    ///
    /// let dt = NaiveDate::from_ymd(1970, 1, 1)?.and_hms_nano(0, 0, 1, 444)?.and_local_timezone(Utc)?;
    ...
    #[inline]
    pub fn checked_add_signed(self, rhs: TimeDelta) -> Result<DateTime<Tz>, Error> {
    ...
    pub fn checked_add_months(self, rhs: Months) -> Result<DateTime<Tz>, Error> {
        self.naive_local()
    ...
    /// Returns `Err(Error)` when it will result in overflow.
    #[inline]
    pub fn checked_sub_signed(self, rhs: TimeDelta) -> Result<DateTime<Tz>, Error> {
    ...
    pub fn checked_sub_months(self, rhs: Months) -> Result<DateTime<Tz>, Error> {
        self.naive_local()
...
impl Default for DateTime<Utc> {
    fn default() -> Self {
        ...
        Utc.from_utc_datetime(&NaiveDateTime::default()).unwrap()
    }
}
...
impl DateTime<Utc> {
    ...
    /// RFC 2822 is the internet message standard that specifies the representation of times in HTTP
    /// and email headers.
    pub fn parse_from_rfc2822(s: &str) -> Result<DateTime<Utc>, Error> {
    ...
    /// also simultaneously valid RFC 3339 values, but not all RFC 3339 values are valid ISO 8601
    /// values (or the other way around).
    pub fn parse_from_rfc3339(s: &str) -> Result<DateTime<Utc>, Error> {
    ...
    Tz::Offset: fmt::Display,
{
    ...
    /// let dt = NaiveDate::from_ymd(2018, 1, 26)?.and_hms_micro(18, 30, 9, 453_829)?.and_local_timezone(Utc)?;
...
#[test]
fn test_add_sub_months() -> Result<(), crate::Error> {
    let utc_dt = Utc.with_ymd_and_hms(2018, 9, 5, 23, 58, 0)?.single()?;
    assert_eq!(utc_dt + Months::new(15), Utc.with_ymd_and_hms(2019, 12, 5, 23, 58, 0)?.single()?);

    let utc_dt = Utc.with_ymd_and_hms(2020, 1, 31, 23, 58, 0)?.single()?;
    assert_eq!(utc_dt + Months::new(1), Utc.with_ymd_and_hms(2020, 2, 29, 23, 58, 0)?.single()?);
    assert_eq!(utc_dt + Months::new(2), Utc.with_ymd_and_hms(2020, 3, 31, 23, 58, 0)?.single()?);

    let utc_dt = Utc.with_ymd_and_hms(2018, 9, 5, 23, 58, 0)?.single()?;
    assert_eq!(utc_dt - Months::new(15), Utc.with_ymd_and_hms(2017, 6, 5, 23, 58, 0)?.single()?);

    let utc_dt = Utc.with_ymd_and_hms(2020, 3, 31, 23, 58, 0)?.single()?;
    assert_eq!(utc_dt - Months::new(1), Utc.with_ymd_and_hms(2020, 2, 29, 23, 58, 0)?.single()?);
    assert_eq!(utc_dt - Months::new(2), Utc.with_ymd_and_hms(2020, 1, 31, 23, 58, 0)?.single()?);
    Ok(())
}

#[test]
fn test_auto_conversion() -> Result<(), crate::Error> {
    let utc_dt = Utc.with_ymd_and_hms(2018, 9, 5, 23, 58, 0)?.single()?;
    let cdt_dt =
        FixedOffset::west(5 * 60 * 60)?.with_ymd_and_hms(2018, 9, 5, 18, 58, 0)?.single()?;
