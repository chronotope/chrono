...
impl TimeZone for DstTester {
    ...
    fn offset_from_local_date(
        &self,
        _: &NaiveDate,
    ) -> Result<crate::LocalResult<Self::Offset>, crate::Error> {
        unimplemented!()
    }
    ...
    fn offset_from_local_datetime(
        ...
        if *local < local_to_winter_transition_end || *local >= local_to_summer_transition_end {
            Ok(LocalResult::Single(DstTester::summer_offset()))
        ...
        {
            Ok(LocalResult::Ambiguous(DstTester::winter_offset(), DstTester::summer_offset()))
        } else if *local >= local_to_summer_transition_start
            && *local < local_to_summer_transition_end
        {
            Err(crate::Error::InvalidDateTime)
        } else {
            Err(crate::Error::InvalidDateTime)
        }
    }

    fn offset_from_utc_date(&self, _: &NaiveDate) -> Result<Self::Offset, crate::Error> {
        unimplemented!()
    }

    fn offset_from_utc_datetime(&self, utc: &NaiveDateTime) -> Result<Self::Offset, crate::Error> {
...
#[test]
fn test_datetime_add_days() -> Result<(), crate::Error> {
    let est = FixedOffset::west(5 * 60 * 60)?;
    let kst = FixedOffset::east(9 * 60 * 60)?;

    assert_eq!(
        format!("{}", est.with_ymd_and_hms(2014, 5, 6, 7, 8, 9)?.single()? + Days::new(5)),
        "2014-05-11 07:08:09 -05:00"
    );
    assert_eq!(
        format!("{}", kst.with_ymd_and_hms(2014, 5, 6, 7, 8, 9)?.single()? + Days::new(5)),
        "2014-05-11 07:08:09 +09:00"
    );

    assert_eq!(
        format!("{}", est.with_ymd_and_hms(2014, 5, 6, 7, 8, 9)?.single()? + Days::new(35)),
        "2014-06-10 07:08:09 -05:00"
    );
    assert_eq!(
        format!("{}", kst.with_ymd_and_hms(2014, 5, 6, 7, 8, 9)?.single()? + Days::new(35)),
        "2014-06-10 07:08:09 +09:00"
    );

    assert_eq!(
        format!("{}", DstTester.with_ymd_and_hms(2014, 4, 6, 7, 8, 9)?.single()? + Days::new(5)),
        "2014-04-11 07:08:09 +09:00"
    );
    assert_eq!(
        format!("{}", DstTester.with_ymd_and_hms(2014, 4, 6, 7, 8, 9)?.single()? + Days::new(10)),
        "2014-04-16 07:08:09 +08:00"
    );

    assert_eq!(
        format!("{}", DstTester.with_ymd_and_hms(2014, 9, 6, 7, 8, 9)?.single()? + Days::new(5)),
        "2014-09-11 07:08:09 +08:00"
    );
    assert_eq!(
        format!("{}", DstTester.with_ymd_and_hms(2014, 9, 6, 7, 8, 9)?.single()? + Days::new(10)),
...
#[test]
fn test_datetime_sub_days() -> Result<(), crate::Error> {
    let est = FixedOffset::west(5 * 60 * 60)?;
    let kst = FixedOffset::east(9 * 60 * 60)?;
...
#[test]
fn test_datetime_add_months() -> Result<(), crate::Error> {
    let est = FixedOffset::west(5 * 60 * 60)?;
    let kst = FixedOffset::east(9 * 60 * 60)?;

    assert_eq!(
        format!("{}", est.with_ymd_and_hms(2014, 5, 6, 7, 8, 9)?.single()? + Months::new(1)),
        "2014-06-06 07:08:09 -05:00"
    );
    assert_eq!(
        format!("{}", kst.with_ymd_and_hms(2014, 5, 6, 7, 8, 9)?.single()? + Months::new(1)),
        "2014-06-06 07:08:09 +09:00"
    );

    assert_eq!(
        format!("{}", est.with_ymd_and_hms(2014, 5, 6, 7, 8, 9)?.single()? + Months::new(5)),
        "2014-10-06 07:08:09 -05:00"
    );
    assert_eq!(
        format!("{}", kst.with_ymd_and_hms(2014, 5, 6, 7, 8, 9)?.single()? + Months::new(5)),
...
#[test]
fn test_datetime_sub_months() -> Result<(), crate::Error> {
    let est = FixedOffset::west(5 * 60 * 60)?;
    let kst = FixedOffset::east(9 * 60 * 60)?;
...
fn ymdhms(
    ...
    fixedoffset.with_ymd_and_hms(year, month, day, hour, min, sec).unwrap().single().unwrap()
}
...
fn ymdhms_utc(year: i32, month: u32, day: u32, hour: u32, min: u32, sec: u32) -> DateTime<Utc> {
    Utc.with_ymd_and_hms(year, month, day, hour, min, sec).unwrap().single().unwrap()
}
...
#[test]
fn test_datetime_offset() -> Result<(), crate::Error> {
    let est = FixedOffset::west(5 * 60 * 60)?;
    let edt = FixedOffset::west(4 * 60 * 60)?;
    let kst = FixedOffset::east(9 * 60 * 60)?;

    assert_eq!(
        format!("{}", Utc.with_ymd_and_hms(2014, 5, 6, 7, 8, 9)?.single()?),
        "2014-05-06 07:08:09 UTC"
    );
    assert_eq!(
        format!("{}", edt.with_ymd_and_hms(2014, 5, 6, 7, 8, 9)?.single()?),
    ...
    let dt = Utc.with_ymd_and_hms(2014, 5, 6, 7, 8, 9)?.single()?;
    assert_eq!(dt, edt.with_ymd_and_hms(2014, 5, 6, 3, 8, 9)?.single()?);
    assert_eq!(
        dt + TimeDelta::seconds(3600 + 60 + 1),
        Utc.with_ymd_and_hms(2014, 5, 6, 8, 9, 10)?.single()?
    );
    assert_eq!(
        dt.signed_duration_since(edt.with_ymd_and_hms(2014, 5, 6, 10, 11, 12)?.single()?),
        TimeDelta::seconds(-7 * 3600 - 3 * 60 - 3)
    );

    assert_eq!(*Utc.with_ymd_and_hms(2014, 5, 6, 7, 8, 9)?.single()?.offset(), Utc);
    assert_eq!(*edt.with_ymd_and_hms(2014, 5, 6, 7, 8, 9)?.single()?.offset(), edt);
    assert!(*edt.with_ymd_and_hms(2014, 5, 6, 7, 8, 9)?.single()?.offset() != est);
    Ok(())
}

#[test]
fn test_datetime_date_and_time() -> Result<(), crate::Error> {
    let tz = FixedOffset::east(5 * 60 * 60)?;
    let d = tz.with_ymd_and_hms(2014, 5, 6, 7, 8, 9)?.single()?;
    assert_eq!(d.time(), NaiveTime::from_hms(7, 8, 9)?);
    assert_eq!(d.date_naive(), NaiveDate::from_ymd(2014, 5, 6)?);

    let tz = FixedOffset::east(4 * 60 * 60)?;
    let d = tz.with_ymd_and_hms(2016, 5, 4, 3, 2, 1)?.single()?;
    assert_eq!(d.time(), NaiveTime::from_hms(3, 2, 1)?);
    assert_eq!(d.date_naive(), NaiveDate::from_ymd(2016, 5, 4)?);

    let tz = FixedOffset::west(13 * 60 * 60)?;
    let d = tz.with_ymd_and_hms(2017, 8, 9, 12, 34, 56)?.single()?;
    assert_eq!(d.time(), NaiveTime::from_hms(12, 34, 56)?);
    assert_eq!(d.date_naive(), NaiveDate::from_ymd(2017, 8, 9)?);

    let utc_d = Utc.with_ymd_and_hms(2017, 8, 9, 12, 34, 56)?.single()?;
    assert!(utc_d < d);
    Ok(())
}
...
#[test]
#[cfg(feature = "clock")]
fn test_datetime_with_timezone() -> Result<(), crate::Error> {
...
#[test]
fn test_datetime_rfc2822() -> Result<(), crate::Error> {
    let edt = FixedOffset::east(5 * 60 * 60)?;

    // timezone 0
    assert_eq!(
        Utc.with_ymd_and_hms(2015, 2, 18, 23, 16, 9)?.single()?.to_rfc2822(),
        "Wed, 18 Feb 2015 23:16:09 +0000"
    );
    // timezone +05
    assert_eq!(
        edt.from_local_datetime(&NaiveDate::from_ymd(2015, 2, 18)?.and_hms_milli(23, 16, 9, 150)?)?
            .single()?
            .to_rfc2822(),
    ...
    assert_eq!(
        edt.from_local_datetime(
            &NaiveDate::from_ymd(2015, 2, 18)?.and_hms_micro(23, 59, 59, 1_234_567)?
        )?
        .single()?
        .to_rfc2822(),
    ...
    assert_eq!(
        ...
        FixedOffset::east(0).unwrap().with_ymd_and_hms(2015, 2, 18, 23, 16, 9)?.single()?
    );
    assert_eq!(
        DateTime::<FixedOffset>::parse_from_rfc2822("Wed, 18 Feb 2015 23:16:09 -0000")?,
    ...
    assert_eq!(
        ...
        ymdhms(&edt, 2015, 2, 18, 23, 59, 58)
    );
    assert_ne!(
        DateTime::<FixedOffset>::parse_from_rfc2822("Wed, 18 Feb 2015 23:59:58 +0500")?,
        ymdhms_milli(&edt, 2015, 2, 18, 23, 59, 58, 500)
    );
...
#[test]
fn test_datetime_rfc3339() -> Result<(), crate::Error> {
    let edt5 = FixedOffset::east(5 * 60 * 60)?;
    let edt0 = FixedOffset::east(0)?;

    // timezone 0
    assert_eq!(
        Utc.with_ymd_and_hms(2015, 2, 18, 23, 16, 9)?.single()?.to_rfc3339(),
        "2015-02-18T23:16:09+00:00"
    );
    // timezone +05
    assert_eq!(
        edt5.from_local_datetime(
            &NaiveDate::from_ymd(2015, 2, 18)?.and_hms_milli(23, 16, 9, 150)?
        )?
        .single()?
        .to_rfc3339(),
    ...
    assert_eq!(
        ...
        edt5.from_local_datetime(
            &NaiveDate::from_ymd(2015, 2, 18)?.and_hms_milli(23, 59, 59, 1_000)?
        )?
        .single()?
    );
    assert!(DateTime::<FixedOffset>::parse_from_rfc2822("31 DEC 262143 23:59 -2359").is_err());
...
#[test]
fn test_rfc3339_opts() -> Result<(), crate::Error> {
...
#[test]
#[should_panic]
fn test_rfc3339_nonexhaustive() {
    use crate::SecondsFormat;
    let dt = Utc.with_ymd_and_hms(1999, 10, 9, 1, 2, 3).unwrap().single().unwrap();
    dt.to_rfc3339_opts(SecondsFormat::__NonExhaustive, true);
}

#[test]
fn test_datetime_from_str() -> Result<(), crate::Error> {
    assert_eq!(
        "2015-02-18T23:16:9.15Z".parse::<DateTime<FixedOffset>>()?,
        FixedOffset::east(0)?
            .from_local_datetime(&NaiveDate::from_ymd(2015, 2, 18)?.and_hms_milli(23, 16, 9, 150)?)?
            .single()?
    );
    assert_eq!(
        "2015-02-18T23:16:9.15Z".parse::<DateTime<Utc>>()?,
        Utc.from_local_datetime(&NaiveDate::from_ymd(2015, 2, 18)?.and_hms_milli(23, 16, 9, 150)?)?
            .single()?
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        FixedOffset::west(10 * 3600)
            .unwrap()
            .from_local_datetime(&NaiveDate::from_ymd(2015, 2, 18)?.and_hms_milli(13, 16, 9, 150)?)?
            .single()?
    );
...
#[test]
fn test_utc_datetime_from_str() -> Result<(), crate::Error> {
    let edt = FixedOffset::east(570 * 60)?;
    let edt0 = FixedOffset::east(0)?;
    let wdt = FixedOffset::west(10 * 3600)?;
    assert_eq!(
    ...
    assert_eq!(
        ...
        Utc.with_ymd_and_hms(2013, 8, 9, 23, 54, 35)?.single()?
    );
...
#[test]
fn test_to_string_round_trip() -> Result<(), crate::Error> {
    let dt = Utc.with_ymd_and_hms(2000, 1, 1, 0, 0, 0)?.single()?;
    let _dt: DateTime<Utc> = dt.to_string().parse()?;

    let ndt_fixed = dt.with_timezone(&FixedOffset::east(3600)?)?;
    let _dt: DateTime<FixedOffset> = ndt_fixed.to_string().parse()?;
...
#[test]
#[cfg(feature = "clock")]
fn test_to_string_round_trip_with_local() -> Result<(), crate::Error> {
    let ndt = Local::now()?;
    let _dt: DateTime<FixedOffset> = ndt.to_string().parse()?;
    Ok(())
}
...
#[test]
#[cfg(feature = "clock")]
fn test_years_elapsed() -> Result<(), crate::Error> {
...
#[test]
fn test_datetime_add_assign() -> Result<(), crate::Error> {
