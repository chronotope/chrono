...
fn test_rkyv_validation() {
    ...
    let bytes = ::rkyv::to_bytes::<::rancor::Error>(&t_min).unwrap();
    assert_eq!(::rkyv::from_bytes::<NaiveTime, ::rancor::Error>(&bytes).unwrap(), t_min);

    let t_max = NaiveTime::MAX;
    let bytes = ::rkyv::to_bytes::<::rancor::Error>(&t_max).unwrap();
    assert_eq!(::rkyv::from_bytes::<NaiveTime, ::rancor::Error>(&bytes).unwrap(), t_max);
}
