...
impl NaiveTime {
    ...
    ///
    /// let from_hmsm = NaiveTime::from_hms_milli;
    /// let since = NaiveTime::abs_duration_since;
    ///
    /// assert_eq!(since(from_hmsm(3, 5, 7, 900), from_hmsm(3, 5, 7, 900)),
    ...
    /// ```
    pub fn abs_duration_since(self, rhs: NaiveTime) -> Duration {
        //     |    |    :leap|    |    |    |    |    |    |    :leap|    |
        //     |    |    :    |    |    |    |    |    |    |    :    |    |
        // ----+----+-----*---+----+----+----+----+----+----+-------*-+----+----
        //          |   `rhs` |                             |    `self`
    ...
    ///
    /// let from_hmsm = NaiveTime::from_hms_milli;
    /// let since = NaiveTime::checked_duration_since;
    ///
    /// assert_eq!(since(from_hmsm(3, 5, 7, 900), from_hmsm(3, 5, 7, 900)),
    ...
    ///            None);
    /// assert_eq!(since(from_hmsm(3, 5, 7, 900), from_hmsm(2, 4, 6, 800)),
    ///            Some(Duration::from_secs(3600 + 60 + 1) + Duration::from_millis(100)));
    /// ```
    ///
    /// Leap seconds are handled, but the subtraction assumes that
    /// there were no other leap seconds happened.
    ...
    /// ```
    pub fn checked_duration_since(self, rhs: NaiveTime) -> Option<Duration> {
        match self >= rhs {
            true => Some(self.abs_duration_since(rhs)),
            false => None,
        }
    }
