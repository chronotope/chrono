...
pub mod ts_seconds {
    ...
    impl<'de> de::Visitor<'de> for SecondsTimestampVisitor {
        ...
        where
            E: de::Error,
        {
            NaiveDateTime::from_timestamp(value, 0).ok_or_else(|| invalid_ts(value))
        }
            ...
            E: de::Error,
        {
            if value > i64::MAX as u64 {
                ...
                NaiveDateTime::from_timestamp(value as i64, 0).ok_or_else(|| invalid_ts(value))
            }
        }
    }
}
...
pub mod ts_seconds_option {
    ...
    ///
    /// let my_s =
    ///     S { time: Some(NaiveDate::from_ymd(2018, 5, 17).unwrap().and_hms(02, 04, 59).unwrap()) };
    /// let as_string = serde_json::to_string(&my_s)?;
    /// assert_eq!(as_string, r#"{"time":1526522699}"#);
    /// # Ok::<(), serde_json::Error>(())
    /// ```
