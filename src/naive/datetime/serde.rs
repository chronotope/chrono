    ...
    fn test_serde_serialize() {
        assert_eq!(
            serde_json::to_string(
                &NaiveDate::from_ymd(2016, 7, 8).unwrap().at_hms_milli(9, 10, 48, 90).unwrap()
            )
            .ok(),
        ...
        assert_eq!(
            serde_json::to_string(
                &NaiveDate::from_ymd(2014, 7, 24).unwrap().at_hms(12, 34, 6).unwrap()
            )
            .ok(),
        ...
        assert_eq!(
            serde_json::to_string(
                &NaiveDate::from_ymd(0, 1, 1).unwrap().at_hms_milli(0, 0, 59, 1_000).unwrap()
            )
            .ok(),
        ...
        assert_eq!(
            serde_json::to_string(
                &NaiveDate::from_ymd(-1, 12, 31).unwrap().at_hms_nano(23, 59, 59, 7).unwrap()
            )
            .ok(),
            Some(r#""-0001-12-31T23:59:59.000000007""#.into())
        );
        assert_eq!(
            serde_json::to_string(&NaiveDate::MIN.at_hms(0, 0, 0).unwrap()).ok(),
            Some(r#""-262143-01-01T00:00:00""#.into())
        );
        assert_eq!(
            serde_json::to_string(&NaiveDate::MAX.at_hms_nano(23, 59, 59, 1_999_999_999).unwrap())
                .ok(),
