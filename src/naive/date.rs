...
impl NaiveDate {
    ...
    pub const MIN: NaiveDate = NaiveDate { inner: DateImpl::MIN };
    /// The maximum possible `NaiveDate` (December 31, 262143 CE).
    pub const MAX: NaiveDate = NaiveDate { inner: DateImpl::MAX };
}

impl Datelike for NaiveDate {
    fn num_days_from_ce(&self) -> i32 {
        i32::try_from(self.inner.signed_duration_since(DateImpl::CE)).unwrap() + 1
    }
    ...
    /// ```
    #[inline]
    fn with_year(&self, year: i16) -> Option<NaiveDate> {
    ...
    /// ```
    #[inline]
    fn with_month(&self, month: u8) -> Option<NaiveDate> {
    ...
    /// ```
    #[inline]
    fn with_month0(&self, month0: u8) -> Option<NaiveDate> {
        NaiveDate::from_ymd_opt(self.year(), month0 + 1, self.day())
    }
    ...
    /// ```
    #[inline]
    fn with_day(&self, day: u8) -> Option<NaiveDate> {
    ...
    /// ```
    #[inline]
    fn with_day0(&self, day0: u8) -> Option<NaiveDate> {
        NaiveDate::from_ymd_opt(self.year(), self.month(), day0 + 1)
    }
    ...
    /// ```
    #[inline]
    fn with_ordinal(&self, ordinal: u16) -> Option<NaiveDate> {
    ...
    /// ```
    #[inline]
    fn with_ordinal0(&self, ordinal0: u16) -> Option<NaiveDate> {
        NaiveDate::from_yo_opt(self.year(), ordinal0 + 1)
    }
}
    ...
    E: ::std::fmt::Debug,
{
    ...
    assert_eq!(to_string(&NaiveDate::MIN).ok(), Some(r#""-32768-01-01""#.into()));
    assert_eq!(to_string(&NaiveDate::MAX).ok(), Some(r#""+32767-12-31""#.into()));
}
    ...
    E: ::std::fmt::Debug,
{
    ...
    assert_eq!(from_str(r#""-32768-01-01""#).ok(), Some(NaiveDate::MIN));
    assert_eq!(from_str(r#""+32767-12-31""#).ok(), Some(NaiveDate::MAX));
...
mod tests {
    use super::{
        internals::{MAX_YEAR, MIN_YEAR},
        Days, Months, NaiveDate, MAX_DAYS_FROM_YEAR_0, MIN_DAYS_FROM_YEAR_0,
    };
    ...
    fn test_date_from_num_days_from_ce() {
        ...
        assert_eq!(
            from_ndays_from_ce(dbg!(NaiveDate::MIN.num_days_from_ce())),
            Some(NaiveDate::MIN)
        );
    ...
    fn test_date_with_fields() {
        ...
        assert_eq!(d.with_year(i16::MAX), None);
        ...
        assert_eq!(d.with_month(u8::MAX), None);
        ...
        assert_eq!(d.with_day(u8::MAX), None);
        ...
        assert_eq!(d.with_ordinal(u16::MAX), None);
    }
    ...
    #[test]
    fn test_date_add() {
        fn check((y1, m1, d1): (i16, u8, u8), rhs: TimeDelta, ymd: Option<(i16, u8, u8)>) {
            let lhs = NaiveDate::from_ymd_opt(y1, m1, d1).unwrap();
            let sum = ymd.map(|(y, m, d)| NaiveDate::from_ymd_opt(y, m, d).unwrap());
            assert_eq!(lhs.checked_add_signed(rhs), sum);

            // otherwise the - will overflow!
            if rhs.num_days() != i64::from(MIN_DAYS_FROM_YEAR_0) {
    ...
    #[test]
    fn test_date_sub() {
        fn check((y1, m1, d1): (i16, u8, u8), (y2, m2, d2): (i16, u8, u8), diff: TimeDelta) {
            let lhs = NaiveDate::from_ymd_opt(y1, m1, d1).unwrap();
    ...
    fn test_date_add_days() {
        fn check((y1, m1, d1): (i16, u8, u8), rhs: Days, ymd: Option<(i16, u8, u8)>) {
            let lhs = NaiveDate::from_ymd_opt(y1, m1, d1).unwrap();
            let sum = ymd.map(|(y, m, d)| NaiveDate::from_ymd_opt(y, m, d).unwrap());
    ...
    fn test_date_sub_days() {
        fn check((y1, m1, d1): (i16, u8, u8), (y2, m2, d2): (i16, u8, u8), diff: Days) {
            let lhs = NaiveDate::from_ymd_opt(y1, m1, d1).unwrap();
            let rhs = NaiveDate::from_ymd_opt(y2, m2, d2).unwrap();
    ...
    fn test_day_iterator_limit() {
        assert_eq!(
            NaiveDate::from_ymd_opt(i16::MAX, 12, 29).unwrap().iter_days().take(4).count(),
            2
        );
        assert_eq!(
            NaiveDate::from_ymd_opt(i16::MIN, 1, 3).unwrap().iter_days().rev().take(4).count(),
            2
        );
    }
    ...
    fn test_week_iterator_limit() {
        assert_eq!(
            NaiveDate::from_ymd_opt(i16::MAX, 12, 12).unwrap().iter_weeks().take(4).count(),
            2
        );
        assert_eq!(
            NaiveDate::from_ymd_opt(i16::MIN, 1, 15).unwrap().iter_weeks().rev().take(4).count(),
            2
        );
    }
