...
fn test_strftime_docs() {
    ...
    assert_eq!(dt.format("%U").unwrap().to_string(), "27");
    ...
    assert_eq!(dt.format("  %Y%d%m%%%%%t%H%M%S\t").unwrap().to_string(), "  20010807%%\t003460\t");
    assert_eq!(
        dt.format("  %Y%d%m%%%%%t%H:%P:%M%S%:::z\t").unwrap().to_string(),
