...
#[inline]
pub(super) fn number(s: &str, min: usize, max: usize) -> Result<(&str, i64), Error> {
    assert!(min <= max);
...
pub(super) fn nanosecond(s: &str) -> Result<(&str, i64), Error> {
    ...
    let v = v.checked_mul(SCALE[consumed]).ok_or(Error::ParsingOutOfRange)?;
...
/// Returns the number of whole nanoseconds (0--999,999,999).
pub(super) fn nanosecond_fixed(s: &str, digits: usize) -> Result<(&str, i64), Error> {
    // record the number of digits consumed for later scaling.
    ...
    static SCALE: [i64; 10] =
        [0, 100_000_000, 10_000_000, 1_000_000, 100_000, 10_000, 1_000, 100, 10, 1];
    let v = v.checked_mul(SCALE[digits]).ok_or(Error::ParsingOutOfRange)?;

    Ok((s, v))
}

/// Tries to parse the month index (0 through 11) with the first three ASCII letters.
pub(super) fn short_month0(s: &str) -> Result<(&str, u8), Error> {
    if s.len() < 3 {
        return Err(Error::ParsingTooShort);
    }
    let buf = s.as_bytes();
...
pub(super) fn short_weekday(s: &str) -> Result<(&str, Weekday), Error> {
    if s.len() < 3 {
        return Err(Error::ParsingTooShort);
    }
    let buf = s.as_bytes();
...
pub(super) fn short_or_long_month0(s: &str) -> Result<(&str, u8), Error> {
...
pub(super) fn short_or_long_weekday(s: &str) -> Result<(&str, Weekday), Error> {
...
pub(super) fn char(s: &str, c1: u8) -> Result<&str, Error> {
    match s.as_bytes().first() {
        Some(&c) if c == c1 => Ok(&s[1..]),
        Some(_) => Err(Error::ParsingInvalid),
        None => Err(Error::ParsingTooShort),
    }
}

/// Tries to consume one or more whitespace.
pub(super) fn space(s: &str) -> Result<&str, Error> {
    let s_ = s.trim_left();
    if s_.len() < s.len() {
        Ok(s_)
    } else if s.is_empty() {
        Err(Error::ParsingTooShort)
    } else {
        Err(Error::ParsingInvalid)
    }
}
...
/// Always returns `Ok(s)`.
pub(super) fn consume_colon_maybe(mut s: &str) -> Result<&str, Error> {
    if s.is_empty() {
        // nothing consumed
        return Ok(s);
    }
...
pub(super) fn timezone_offset<F>(s: &str, consume_colon: F) -> Result<(&str, i32), Error>
where
    F: FnMut(&str) -> Result<&str, Error>,
{
...
) -> Result<(&str, i32), Error>
where
    F: FnMut(&str) -> Result<&str, Error>,
{
    const fn digits(s: &str) -> Result<(u8, u8), Error> {
        let b = s.as_bytes();
        if b.len() < 2 {
            Err(Error::ParsingTooShort)
        } else {
    ...
    let minutes = if let Ok(ds) = digits(s) {
        match ds {
            ...
            (b'6'..=b'9', b'0'..=b'9') => return Err(Error::ParsingOutOfRange),
            _ => return Err(Error::ParsingInvalid),
        }
    ...
    s = match s.len() {
        len if len >= 2 => &s[2..],
        len if len == 0 => s,
        _ => return Err(Error::ParsingTooShort),
    };
...
pub(super) fn timezone_offset_zulu<F>(s: &str, colon: F) -> Result<(&str, i32), Error>
where
    F: FnMut(&str) -> Result<&str, Error>,
{
    let bytes = s.as_bytes();
...
pub(super) fn timezone_offset_permissive<F>(s: &str, colon: F) -> Result<(&str, i32), Error>
where
    F: FnMut(&str) -> Result<&str, Error>,
{
    match s.as_bytes().first() {
...
pub(super) fn timezone_name_skip(s: &str) -> Result<(&str, ()), Error> {
...
pub(super) fn comment_2822(s: &str) -> Result<(&str, ()), Error> {
    use CommentState::*;
...
#[test]
fn test_space() {
    assert_eq!(space(""), Err(Error::ParsingTooShort));
    assert_eq!(space(" "), Ok(""));
    assert_eq!(space(" \t"), Ok(""));
    assert_eq!(space(" \ta"), Ok("a"));
    assert_eq!(space(" \ta "), Ok("a "));
    assert_eq!(space("a"), Err(Error::ParsingInvalid));
    assert_eq!(space("a "), Err(Error::ParsingInvalid));
}
